document.addEventListener('DOMContentLoaded', () => {
   ...
   // --- 1. СИСТЕМА МАСШТАБИРОВАНИЯ (ИСПРАВЛЕНА) ---
    ...
    function resizeGame() {
        ...
        // Рассчитываем масштаб, берём минимальный коэффициент
        ...
        // Переключаем точку трансформации строго на центр холста
        gameContainer.style.transformOrigin = 'center center';

        // Идеальное центрирование внутри экрана смартфона
    ...
    // Дополнительно вешаем на событие смены ориентации экрана
    window.addEventListener('orientationchange', () => setTimeout(resizeGame, 200));
    resizeGame();


    // --- 2. ПРЕЛОАДЕР + ЗАГРУЗКА АССЕТОВ (ИСПРАВЛЕНА) ---
    ...
    setTimeout(() => {
        if (preloader) {
            ...
            setTimeout(() => {
                ...
                // Принудительный ресайз после очистки DOM
                resizeGame(); 
            }, 700);
        }
        // Даем Telegram WebApp 150мс на завершение анимации развертывания
